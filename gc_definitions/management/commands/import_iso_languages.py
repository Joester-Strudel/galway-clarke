...
# First-Party Imports
from gc_definitions.models import IsoLanguage, IsoLanguageScope, IsoLanguageType


class Command(BaseCommand):
    help = (
        "Scrape ISO 639 data from iso639-3.sil.org and create/update ISO language records."
    )
    ...
    def handle(self, *args, **options):
        ...
        while current_url and current_url not in seen_pages:
            ...
            headers = [
                self._normalize_header(th.get_text())
            ...
            for row in rows:
                ...
                name = self._get_column_value(
                    headers, data, ["ref name", "reference name", "name"]
                )
                iso1 = self._get_column_value(
                    headers, data, ["639-1", "iso639-1"]
                )
                ...
                type_value = self._get_column_value(headers, data, ["type", "language type"])

                iso2, iso5 = self._parse_iso2_iso5(combined_2_5)

                # Ensure scope/type records exist (even if not linked yet)
                if scope_value:
                    IsoLanguageScope.objects.get_or_create(name=scope_value)
                if type_value:
                    IsoLanguageType.objects.get_or_create(name=type_value)
    ...
    def _find_next_page(self, soup, current_url):
        ...
        # Try Drupal-style pager item
        pager_next = soup.find("li", class_=re.compile("pager__item--next"))
        if pager_next:
